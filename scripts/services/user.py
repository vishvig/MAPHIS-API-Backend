...
from scripts.errors.exceptions import MaphisException

# Importing API related requests/response validators
from scripts.schemas.user import (AddUserRequest, DeleteUserRequest, UpdateUserRequest)
...
async def delete_user(request_data: AddUserRequest):
    try:
        response_json = user_handler.delete_user(request_data=request_data)
        return DefaultResponse(status="success", message=response_json)
    except ValidationError as e:
        LOG.error(f"Request data model validation failed: {e.json()}")
        return DefaultFailureResponse(
...
async def update_user(request_data: AddUserRequest):
    try:
